...
|---|---:|---:|
| 残った宣言(全破壊) | 96.1% | 95.9% |
| きれいに回復した破壊(壊した箇所以外を失わず、余計なものも出さない) | 92.1% | 90.6% |
| きれいに回復、`insert {` | 90.4% | 89.7% |
| きれいに回復、`delete }` | 82.2% | 83.7% |
| きれいに回復、`delete )` | 98.1% | 93.9% |
| きれいに回復、`insert (` | 97.7% | 94.8% |

4 通り中 3 通りと全体で gramide が上。tree-sitter が多く残すのはメソッドの `}` を消してクラス本体が
閉じなくなる場合で、tree-sitter は欠けた波括弧をあるべき位置に置き、gramide はクラスを落とすか、
残して後続を入れ子にする。
