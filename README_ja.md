...
なかった破壊を「きれい」と数える([証拠](docs/evidence/recovery-node-lib.json)、[仕組み](https://github.com/O6lvl4/gramide/blob/main/docs/recovery.md)):

| Node `lib/`: 427 ファイル、1,694 回の破壊 | gramide | tree-sitter |
|---|---:|---:|
| 残った宣言(全破壊) | 99.4% | 95.9% |
| きれいに回復した破壊(壊した箇所以外を失わず、余計なものも出さない) | 98.3% | 90.6% |
| きれいに回復、`insert {` | 97.9% | 89.7% |
| きれいに回復、`delete }` | 96.6% | 83.7% |
| きれいに回復、`delete )` | 99.5% | 93.9% |
| きれいに回復、`insert (` | 99.3% | 94.8% |

4 通りすべてで gramide が上。以前 tree-sitter が多く残していたメソッドの `}` を消す場合も、今は
メソッドの本体がメソッドと同じ深さにある次のメンバの手前で終わり、クラスは読み続けられる。
その破壊の 96.6% がきれいで、tree-sitter は 83.7%、以前は 82.2% だった。
