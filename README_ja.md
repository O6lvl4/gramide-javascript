...
|---|---:|---:|
| 残った宣言(全破壊) | 94.3% | 95.9% |
| きれいに回復した破壊(壊した箇所以外を失わず、余計なものも出さない) | 89.7% | 90.6% |
| きれいに回復、`insert {` | 89.0% | 89.7% |
| きれいに回復、`delete }` | 77.2% | 83.7% |
| きれいに回復、`delete )` | 95.0% | 93.9% |
| きれいに回復、`insert (` | 97.2% | 94.8% |

全体では互角。差は、メソッドの `}` を消してクラス本体が閉じなくなる場合で、tree-sitter は
欠けた波括弧をあるべき位置に置き、gramide はクラスを落とすか、残して後続を入れ子にする。
